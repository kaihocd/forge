...
function ensureBrew() {
  ...
  if (result.error) throw new Error(`${errTag()} ${red('missing brew: command not found')}`);
  if (result.status !== 0)
...
function isTapped(tap: string) {
  ...
  if (result.status !== 0) throw new Error(`${errTag()} ${red('brew tap: failed to list taps')}`);
...
try {
  ...
  for (const tap of taps) {
    if (isTapped(tap)) console.log(`${skipTag()} tap ${tap}: already tapped`);
    else {
      console.log(`${okTag()} tap ${tap}: installing`);
      install(['tap', tap], `tap ${tap}`);
    }
    if (isTrusted(tap)) console.log(`${skipTag()} tap ${tap}: already trusted`);
    else {
  ...
  for (const cask of casks) {
    if (isInstalled('cask', cask)) console.log(`${skipTag()} cask ${cask}: already installed`);
    else {
