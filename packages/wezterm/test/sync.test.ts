...
async function temporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(path.join(tmpdir(), 'wezterm-sync-'));
...
describe('wezterm sync', () => {
  it('writes a plan linking the WezTerm config directory', async () => {
    const planDirectory = await temporaryDirectory();
    await runSync(planDirectory);
    const entries = JSON.parse(
      await readFile(path.join(planDirectory, 'wezterm.json'), 'utf8'),
    ) as Array<{ name: string; source: string; target: string }>;

    expect(entries).toEqual([
      {
        name: 'wezterm/main',
        source: path.join(packageRoot, 'files'),
        target: path.join(homedir(), '.config', 'wezterm'),
      },
    ]);
  });
});
