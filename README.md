...
The repository currently contains the automation engine only: a symlink
installer, a builder pipeline, and repo tooling. No real config sources have
been migrated yet; `forge.config.yaml` ships as commented examples with empty
task lists.
...
`forge.config.yaml` declares what gets installed, as `links` (symlink as-is) and
`build` (compile a template) tasks. It currently ships with commented examples
and empty task lists; the first real configs (Neovim, zshrc) get migrated in
later. There is no workspace or multi-package structure by design.
...
`pnpm start` runs `build` then `sync`. It expects local dependencies to
already be installed; if `node_modules` is missing, it will ask you to run
`pnpm install` first.

## Build vs Sync

Forge installs config in two ways, declared in `forge.config.yaml`:

- `sync` symlinks repo files that are usable as-is, so edits in the repo take
  effect immediately. Example: `configs/nvim/` -> `~/.config/nvim`.
- `build` compiles templates that need transformation, then writes the result
  directly to its target. Example: `configs/zsh/zshrc` -> `~/.zshrc`, with shell
  aliases injected at build time.
...
- `builder`: which builder handles it (`zshrc` is currently the only one)
- `source`: the input, interpreted by the builder (a template file here)
- `output`: where the built file lands
- `opts`: builder-specific data (the `zshrc` builder takes an `aliases` map and
  injects it at the `{{ aliases }}` placeholder)

Built files carry a `Generated by Forge` marker. The build refuses to overwrite
